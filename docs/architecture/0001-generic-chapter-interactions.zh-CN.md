...
截至 2026-07-22，协议、摘要缓存、相邻章节预加载、模糊锚点、真实分页投影、三级入口、分页下拉手势、滚动模式菜单入口、来源动作和隔离 WebView 均已实现。LegadoHub 已作为首个书源适配，增强客户端保持纯正文，普通客户端仅保留书源生成的章末兼容入口。
...
```javascript
java.hasReaderCapability("chapter-comments", 1)
```

约束：

- 未知能力返回 `false`，不抛异常。
- 普通客户端不存在该方法，书源规则必须用 `try/catch` 降级。
...
- 定制客户端：能力探测成功，正文保持纯净，由原生三级评论能力按书源配置显示段级、页级和章末入口。
- 普通客户端：能力探测失败，不插入页热评，只在章末追加现有可点击入口。

普通客户端的章末入口仍由书源规则生成；客户端不需要识别新字段。`ruleContent` 在一次执行中先完成能力探测，再原子选择“纯正文”或“正文加章末入口”，禁止先追加入口再异步删除。定制客户端不得同时保留该兼容入口，避免重复。

旧客户端导入时会忽略 `chapterComment`，再次导出书源时也可能丢失该未知字段。这不影响章末降级，但从普通客户端迁移到定制客户端时必须重新从原始订阅 URL 导入，不能把普通客户端导出的 JSON 当作完整增强书源。

书源更新不会自动清理普通客户端已保存的旧正文缓存。移除旧页气泡的版本必须在发布说明中明确要求普通客户端执行一次“清除本书缓存”。定制客户端给正文缓存增加规则指纹 sidecar：无指纹或指纹变化时优先重抓当前章和预加载窗口；离线时可以回退旧正文，但必须把它标为 stale，恢复网络后重新抓取。

## 缓存与预加载

- 评论摘要与正文缓存分离。
- 缓存键包含书源 URL、`ContentRule.chapterComment` 指纹、书籍 URL、章节 URL和协议版本。
- 正文缓存 sidecar 保存 `ruleContent` 指纹，用于避免书源升级后继续展示旧的兼容入口；它不改变正文缓存文件格式。
